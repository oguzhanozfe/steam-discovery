...
export function ReadingRoom({ query, setQuery, topic, setTopic, explore, articleId }: { query: string; setQuery: (q: string) => void; topic: ReadingTopic; setTopic: (t: ReadingTopic) => void; explore: (q: string) => void; articleId?: string }) {
  ...
  const addedNotes = latestReadingUpdates.articles.filter(article => latestReadingUpdates.latestBatch.articleIds.includes(article.id));
  const newlyPublishedCount = addedNotes.filter(article => article.date >= latestReadingUpdates.latestBatch.publishedSince).length;
  ...
  return <section className="workbench content-view reading-view">
    <div className="workspace-heading"><div><p className="section-kicker"><BookOpen /> THE PC MARKET READING ROOM</p><h1>{articleId ? readingLibrary.find(article => article.id === articleId)?.title : <>Read the evidence.<br />Keep the disagreement.</>}</h1></div><p className="workspace-note">{readingLibrary.length} article notes: {htmagePosts.length} HTMAG posts + {newsletterLibrary.articles.length} analyst, practitioner and developer readings. {recentCount} are in the original June–September research window. Sorted by source publication date; verification dates and access limits are shown separately.</p></div>
    {!articleId && <div className="reading-freshness"><strong>Source check: <time dateTime={readingUpdatedAt}>{readingUpdatedAt}</time></strong><span>{addedNotes.length} notes added: {newlyPublishedCount} newly published, {addedNotes.length - newlyPublishedCount} archive additions.</span><a href="/feed.xml">Subscribe via RSS</a><a href="/data/readings.json">Download source metadata</a><a href="/about/#sources">Source check ledger</a></div>}
    {!articleId && <section className="reading-expansion" aria-labelledby="reading-expansion-heading"><div><p className="section-kicker">READING PATH · FIVE PERSPECTIVES</p><h2 id="reading-expansion-heading">{newReadingCount} practical readings. From a clear hook to a credible test.</h2><p>Game World Observer, Derek Lieu, Game If You Are, IMPRESS and Game Developer—with an earlier set of 26 proposed actions for a five-person demo team. For the new solo and small-team action plans, open Developer Playbooks. All plans are editorial proposals, not proven recipes.</p></div><ol>{readingStarterPath.map(step => <li key={step.articleId}><a href={readingPath(step.articleId)}>{step.label}<ArrowUpRight /></a><span>{step.detail}</span></li>)}</ol></section>}
    ...
    <div className="publication-pills" aria-label="Filter by publication"><button className={!query && topic === 'All topics' ? 'active' : ''} onClick={() => { setQuery(''); setTopic('All topics'); }}>All publications</button>{publications.map(name => <button key={name} className={query === name ? 'active' : ''} aria-pressed={query === name} onClick={() => { setQuery(name); setTopic('All topics'); }}>{publicationLabel(name)} <span>({readingLibrary.filter(article => article.publication === name).length})</span></button>)}</div>
      ...
      {article.applicationSteps && <details className="reading-application" open={!!articleId}><summary>Apply in a small-team experiment · {article.applicationSteps.length} steps</summary><p>Our proposed experiment—not a result measured by this article.</p><ol>{article.applicationSteps.map(step => <li key={step}>{step}</li>)}</ol></details>}
