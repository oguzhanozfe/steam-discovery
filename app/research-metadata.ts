...
export function citationUrls(route: SiteRoute) {
  ...
  return [...new Set([...(route.citations ?? []), ...extra, ...proposals, ...guides, ...(route.initial.view === 'methodology' ? advertisingReferences.map(ref => ref.url) : [])])];
}
...
export function structuredData(route: SiteRoute) {
  ...
  const editorial = !!(article || story || guide);
  const parentPath = guide ? '/playbooks/' : article ? '/reading/' : story ? '/stories/' : route.initial.gameId ? '/games/' : '/';
  ...
  if (editorial) graph.push({
    ...
    headline: article ? `Research note: ${article.title}` : story?.headline ?? guide?.title,
    description: article?.summary ?? story?.deck ?? guide?.synopsis,
